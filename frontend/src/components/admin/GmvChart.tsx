...
import styles from "./GmvChart.module.css";
...
export default function GmvChart() {
  ...
  useEffect(() => {
    if (!token) return;
    let cancelled = false;
    get<RevenueSeries>(`/api/v1/admin/gmv-series?range=${range}`, token)
      .then((d) => {
        if (cancelled) return;
        setData(d);
        setError(false);
  ...
  return (
    <section className={styles.card}>
      <div className={styles.head}>
        <div>
          <h2 className={styles.title}>{t("gmvOverview")}</h2>
          <p className={styles.sub}>{t("gmvSubtitle")}</p>
        </div>
      ...
      {error ? (
        ...
        <div className={styles.empty}>{t("gmvEmpty")}</div>
      ) : (
