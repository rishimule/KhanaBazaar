/**
 * Khana Bazaar — Client-Side Cart Manager
 *
 * Manages shopping carts in localStorage using the guest session_id pattern.
...
export function removeFromCart(storeId: number, productId: number): Cart[] {
  ...
  if (cart) {
    ...
    // Remove the cart entirely if empty
...
/** Get the total number of items across all carts. */
export function getCartCount(): number {
  return getAllCarts().reduce(
...
/** Calculate the grand total across all carts. */
export function getGrandTotal(): number {
  return getAllCarts().reduce((sum, cart) => sum + getCartTotal(cart), 0);
}
