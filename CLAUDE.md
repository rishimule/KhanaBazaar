...
- `VAPID_PRIVATE_KEY`, `VAPID_PUBLIC_KEY`, `VAPID_SUBJECT` (Web Push; all optional — push is disabled/no-op when `VAPID_PRIVATE_KEY` is empty). Private key stored single-line with escaped `\n`; the worker restores real newlines at send time. Keygen in `docs/development_guide.md` §12.
