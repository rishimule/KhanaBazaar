...
| Email | `EMAIL_PROVIDER=console` (dev) or `brevo+console` (prod, raw httpx call — no SDK) |
...
./scripts/native_infra.sh setup                               # One-time alt: same services, no Docker, no root
./scripts/dev.sh start                                        # Full stack; auto-detects docker vs native infra
```
...
| `/catalog/admin` | `catalog_bulk.py` | Bulk catalog ops: CSV import (`/imports*`), `export.csv`, `imports/template.csv`, multi-select `bulk/status` |
...
Admin-only: create categories/products, approve seller applications, **per-seller supervisor** actions (`/admin/*`), **bulk catalog CSV import/export** (`/catalog/admin/*`).
...
- `COMPANY_NAME` (default `Khanabazaar`) — single source for the displayed brand name. `EMAIL_BRAND_NAME` and the OpenAPI `PROJECT_NAME` derive from it when unset (an explicit `EMAIL_BRAND_NAME`/`PROJECT_NAME` still overrides). Drives email + SMS + WhatsApp-mock copy. NOTE: production WhatsApp text comes from Twilio-approved templates, not this var. **Prod (GCP `sarvaka-prod`) is pinned to `Sarvaka`**: api via `--update-env-vars=COMPANY_NAME` in `deploy.yml`; the VM worker (order/delivery emails+SMS) via `COMPANY_NAME` in `/opt/kb/.env`.
- `EMAIL_PROVIDER` (`console` default | `resend` | `brevo` | `smtp`, plus a `<transport>+console` composite of each that sends **and** captures to `/dev-emails`). Unknown values fall through to `console` **silently** — a typo stops real mail with no error.
- `RESEND_API_KEY`, `RESEND_FROM_EMAIL` (only when `EMAIL_PROVIDER=resend`)
- `BREVO_API_KEY`, `BREVO_FROM_EMAIL` (only when `EMAIL_PROVIDER=brevo`/`brevo+console`). From-address must be verified in Brevo → Senders & Domains or the API 400s. Sender *name* comes from `EMAIL_BRAND_NAME`. **Prod (GCP `sarvaka-prod`) runs `brevo+console`**: api via `--update-env-vars` in `deploy.yml` + `--update-secrets=BREVO_API_KEY`; the VM worker via `/opt/kb/.env` (hand-edited — the deploy never rewrites it).
- `EMAIL_DEV_PREVIEW_DIR` (default `""` = disabled **everywhere**, including development) — directory for dev-only HTML previews of outbound email. Deliberately not derived from `ENVIRONMENT`: prod runs `ENVIRONMENT=development` so the dev mailbox works, and an `ENVIRONMENT` gate mirrored every HTML body (OTP codes included) into the Cloud Run container's in-memory `/tmp`.
- `SUPPORT_EMAIL` (default `support@khanabazaar.example`) — destination inbox for `/customers/me/support` messages
- `EMAIL_FRONTEND_BASE_URL` (default `http://localhost:3000`) — public web origin used to build **every** clickable link in outbound comms: all email templates (via `email_render._global_ctx` → `frontend_base_url`) and the referral invite SMS (`worker._referral_activation_url`); referral-over-WhatsApp will use it at go-live. Existing WhatsApp templates carry no links. **Prod (GCP `sarvaka-prod`) is pinned to `https://app.sarvakaecommerce.com`** in BOTH the api (via `--update-env-vars` in `deploy.yml`) and the VM worker (`/opt/kb/.env` — added by hand, mirroring `COMPANY_NAME`, since the deploy only restarts the worker and never rewrites its `.env`). No trailing slash.
...
| `schema.sql` | Full Postgres schema snapshot — source of truth is the SQLModel models + Alembic; pinned to head `9bb307d813e9`. Regenerate when the head changes. |
...
**Email dispatch**: OTP and order emails sent via Celery tasks (`worker.py`). Provider switch in `core/email.py` — `console` logs to stdout, `resend`/`brevo` do direct httpx POSTs (no SDK dep). **Brevo is not a Resend clone**: it authenticates with an `api-key` header (not `Authorization: Bearer`) and its payload keys differ throughout (`sender`/`to[{email}]`/`textContent`/`htmlContent`/`replyTo{email}`). `BrevoWithConsoleSender` catches `httpx.HTTPError` — wider than `ResendWithConsoleSender`'s `HTTPStatusError` — so timeouts and DNS failures are swallowed too and a provider outage degrades to console-capture instead of breaking OTP login. A temporary local-dev **Gmail SMTP** provider is also available — `EMAIL_PROVIDER=smtp` (send only) or `smtp+console` (send + `/dev-emails` capture); config via `SMTP_*` env vars (App Password required), `SmtpEmailSender`/`SmtpWithConsoleSender` in `core/email.py`. The `+console` composites tag the dev-mailbox `provider` column with the real transport (`smtp`/`resend`). **Two email paths honor `EMAIL_PROVIDER` independently**: the API request path via `get_email_sender()` (OTP request), and the Celery worker path via `worker._resolve_email` (order/notification/referral/support emails) — the latter has its own sync provider switch (`_send_email_via_transport`) and bridges to `SmtpEmailSender` on a worker thread. Wiring only one is the classic half-migration here. **Capture ordering differs by provider shape**: `_resolve_email` writes the `/dev-emails` row (`_capture_dev_email`) *before* the send for `+console` composites and swallows transport errors — so the record survives a provider outage and Celery does not retry a send already recorded — but *after* the send for plain transports, so failures propagate into `autoretry_for=(Exception,)`.
...
**Local infra has two interchangeable backends**: `scripts/dev.sh` supports Docker Compose *or* a Docker-free user-space stack (`scripts/native_infra.sh` — Postgres 15 + PostGIS 3.4, Redis 7, Meilisearch v1.11 from conda-forge/micromamba plus the upstream Meilisearch binary, all under `~/.local/share/khanabazaar`, no root). Same ports and credentials either way, so `.env` is unchanged. `dev.sh` auto-picks native when that env is provisioned (an explicit opt-in beats a merely-installed Docker); override with `KB_INFRA_MODE=docker|native`. Infra state is machine-global like the old named volumes — never assume it is per-worktree. All Docker/native branching is confined to the `infra_*` functions in `dev.sh`; don't reintroduce bare `docker compose` calls into `cmd_*`. Liveness is decided by **port probes, not pid files** (`setsid` can fork and leave a stale pid). See `docs/local_setup.md` §3b.
...
**Customer account lifecycle (soft-delete)**: `User.account_status` (native PG enum `accountstatus`, lowercase members `active`/`deactivated`/`suspended`/`deleted`) is the source of truth; `is_active` is a **synced mirror** (`is_active == status==active`). Every transition funnels through `services/account_lifecycle.transition()` (row-locked `FOR UPDATE`, validates a fixed legal-transition set, guards open obligations, writes a `customer_account_event` row, revokes all sessions when leaving `active`) — it **flushes; callers commit**, then dispatch the email **after commit** (capture `user.id`/`email` into locals first — post-commit ORM access is `MissingGreenlet`). Customer self-service: `POST /customers/me/deactivate` (reversible), `POST /customers/me/delete/otp/request` + `POST /customers/me/delete` (terminal, fresh-OTP-confirmed via namespace `account_delete`). Admin supervisor (`api/admin_actions.py`, keyed by `customer_profile_id`, guards `role==Customer`): `/admin/customers` list/hub/activity + read-only orders/addresses/notifications viewers + `suspend`/`unsuspend`/`delete`/`restore` (reason ≥10 chars). **Auth gates** (three sites): `otp_verify` blocks `deleted`/`suspended` (403) and silently reactivates `deactivated`→`active` on login; `get_current_user` rejects non-`active` (403, kills a live short-lived access token mid-session); `/auth/refresh` rejects non-`active` (a deactivated user reactivates only via fresh OTP login, never refresh). **Open obligations** (non-terminal order OR `CreditAccount.outstanding_balance>0`) block self-service deactivate/delete only — admin bypasses (abuse response). **No PII scrub, no hard delete, ever.** Audit lives in `customer_account_event` (NOT `admin_audit` — its `target_seller_id` is a NOT NULL FK to sellerprofile). 5 English-only emails via one param'd task `send_account_status_email_async(user_id, event_key)`. Spec: `docs/superpowers/specs/2026-07-20-customer-account-lifecycle-design.md`.
...
**Returns (BRD-driven, `docs/superpowers/specs/2026-08-18-return-order-management-design.md`)**: A delivered order can be returned in full or by complete item lines inside a per-service window. **Three OTPs, two storage mechanisms**: the customer-typed *initiation* and *payment-receipt* codes live in Redis (`core/otp.py` namespaces `return_initiate` / `return_payment`, identifier `{user_id}:{return_id}`), while the seller-typed *handover* code lives in `return_request.receipt_otp` columns — it must survive until the customer physically reaches the store, which a 10-minute Redis TTL cannot do. `_serialize(include_receipt_otp=)` exposes that code **only** to the owning customer and **only** while `active`; sellers type what the customer shows them. **Three credit directions exist — keep them straight**: `CreditAccount` = customer owes seller (postpaid), `Store.fee_credit_balance` = platform owes seller (fee wallet), `CustomerStoreCredit` = seller owes customer (returns). **Settlement order is fixed** (`services/return_settlement.settle`): reverse postpaid debt first, capped at the *current* `outstanding_balance` (repayments aren't allocated per order, so "how much of this order is still owed" is unanswerable), then the remainder follows the customer's choice — store credit closes immediately, cash parks in `awaiting_payment_confirmation`. A return fully absorbed by reversal closes even if the customer asked for money back. **The delivery fee comes back only on a full-order return** — one request covering every line with no prior accepted return. **`return_window_days` lives on `SellerProfileService` with its OWN endpoints** (`PATCH /sellers/me/services/{id}/returns`, `PATCH /sellers/admin/{seller_id}/services/{id}/returns`) modelled on the *pause* routes: the general services PATCH rejects approved sellers with `use_change_request`. `0` disables returns for that service. **Item lines lock** while a return is non-terminal or `closed`; `rejected`/`withdrawn`/`expired` release them (`LINE_LOCKING_STATUSES`). Two Celery-swept expiry stages (`returns.sweep_expired`, hourly at minute 12). Admin force paths bypass the seller OTP with a reason ≥10 chars, audited in the same transaction; they never bypass the customer's consent OTP. Open returns count as an account-lifecycle obligation; a positive store-credit balance deliberately does **not** (it's the customer's asset, not a debt). No rewind guard is needed — `rewind_order` and `cancel_order` already reject `Delivered`.

**Bulk catalog import (CSV, `services/catalog_{csv,import}.py`, `api/catalog_bulk.py`)**: Two-phase. `POST /catalog/admin/imports` parses + validates + stages one `catalog_import_row` per CSV line and **writes nothing to the catalog**; `POST /imports/{id}/apply` hands the staged rows to Celery (`catalog.apply_catalog_import`). One wide CSV, one row per product carrying its whole `service → category → subcategory → product` path; the importer upserts every level. **Slugs are required and ARE the identity** — a rename is "same slug, new name", and the corollary is that CSV **cannot move a product between subcategories** (a different path reads as a new product; moves stay in the per-row UI). **Import never deactivates** — bulk deactivation is the separate, confirmed `POST /catalog/admin/bulk/status` behind the admin table's multi-select, which (like the per-row Deactivate) does **not** cascade to children. A soft-deleted row is **reactivated, not forked** (forced anyway for `Service`, whose `slug` is globally unique). An **unknown column is a file-level 422, not a silent ignore** — a typo'd `base_prices` would otherwise import every product at the wrong price. `image_url` is only written on create, or on update when the product has **no** images at all, honouring the `MasterProductImage`-owns-the-cover invariant that makes `PUT /products/{id}` refuse that field. `GET /catalog/admin/export.csv` emits the importer's exact column set — that round trip is how bulk *editing* works, and `tests/test_catalog_export.py` pins "unedited export re-imports as all-noop" (diffs compare `_format_price` output on both sides so float representation can't break it). Apply commits per `APPLY_CHUNK` (200) rows and stamps `applied_at` per row, so it resumes from `failed`; a SIGKILLed worker leaves a job stuck in `applying` and the documented recovery is re-uploading the same file, not racing a second worker. `catalog_import_job` doubles as the **audit record** — `admin_action_log.target_seller_id` is a NOT NULL FK to `sellerprofile` and the catalog is not seller-scoped (same wall the customer-lifecycle work hit). **Do not try to "batch" the Meilisearch sync here**: `search.reindex_products_by_subcategory` is a fan-out *dispatcher*, not a bulk indexer, so enqueuing one per touched subcategory re-indexes every product in it — the standard `after_commit` hook is already proportional (deduped per commit) and handles parent renames. Caps: 5,000 rows / 5 MB per file, 500 ids per `bulk/status`. The import page is **English-only** (operator tool; CSV column names are English regardless), following the `Wallet credit` precedent.
