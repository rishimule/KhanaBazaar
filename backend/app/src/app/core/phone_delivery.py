...
"""Phone-message channel routing: WhatsApp-preferred, SMS fallback.
...
async def deliver_phone_message(
    *,
    ...
    """Send over WhatsApp if enabled, else/then SMS. Returns the channel
    ...
    if whatsapp_sender is not None:
        try:
            ...
            logger.warning(
                "WhatsApp send failed (template=%s); falling back to SMS",
