...
- **DSN must be `postgresql+asyncpg://`.** Cloud SQL connection strings come as a plain `postgresql://` host (or `postgres://` from Heroku-style envs); the validator rewrites both, but a hand-written `postgresql://` without rewriting still fails. Be explicit.
