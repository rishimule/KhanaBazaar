...
**`VAPID_SUBJECT` must be a REAL, deliverable contact** — a `mailto:` with a real domain (e.g. `mailto:you@yourdomain.com`) or a real `https://` URL. Apple's push service (`web.push.apple.com`) validates it and **rejects placeholder domains** — `mailto:support@khanabazaar.example` (and any `.example` address) returns `403 {"reason":"BadJwtToken"}` and the push silently fails. The config default is a placeholder; override it in `.env`. (Google/FCM is lenient here, so this bites only Safari/Apple devices.)
